...
7. Recalculate with a supported spreadsheet calculation engine when available; otherwise validate formulas structurally and disclose that cached values require recalculation in Excel/Google Sheets.
...
- Add a list data-validation dropdown for the scenario selector with values 1, 2, 3 (or Base, Bull, Bear) using the runtime’s spreadsheet validation API.
